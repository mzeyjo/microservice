...
class ShopViewSet(viewsets.ViewSet):
    def list(selt, request): # /api/shop
    ...
    def update(self, request, pk=None): # /api/shop/<str:idx>
        shop = Shop.objects.get(id=pk)
        serializer = ShopSerializer(instance = shop, data = reqeust.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
...
class OrderViewSet(viewsets.ViewSet):
    def list(selt, request): # /api/order
    ...
    def update(self, request, pk=None): # /api/order/<str:idx>
        order = Order.objects.get(id=pk)
        serializer = OrderSerializer(instance = order, data = reqeust.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_202_ACCEPTED)
